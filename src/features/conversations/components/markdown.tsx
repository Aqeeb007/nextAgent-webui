...
const components: Components = {
  ...
  h1: ({ children }) => (
    <h1 className="mb-2.5 text-base font-semibold last:mb-0">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="mb-2.5 text-[0.95rem] font-semibold last:mb-0">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="mb-2 text-sm font-semibold last:mb-0">{children}</h3>
  ),
  hr: () => <hr className="my-2.5 border-current/20" />,
  blockquote: ({ children }) => (
    <blockquote className="mb-2.5 border-l-2 border-current/30 pl-3 italic last:mb-0">
      {children}
    </blockquote>
  ),
  pre: ({ children }) => (
    <pre className="mb-2.5 overflow-x-auto rounded-md bg-background/60 p-2.5 last:mb-0">
      {children}
    </pre>
  ),
